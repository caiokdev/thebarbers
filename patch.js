...
const code = fs.readFileSync('src/hooks/useDashboardData.js', 'utf-8');
...
for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes('Faturamento do Dia') && lines[i].includes('status=closed')) {
        startIndex = i;
    }
    if (lines[i].includes('NOVO: Clientes Inadimplentes') && lines[i].includes('is_subscriber') && lines[i].includes('overdue')) {
        endIndex = i;
    }
}
...
fs.writeFileSync('src/hooks/useDashboardData.js', newContent);
console.log('PATCH_STRATEGY_OK');
