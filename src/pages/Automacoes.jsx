...
export default function Automacoes() {
        ...
        async function fetchFailedLogs() {
            ...
            try {
                const { data, error } = await supabase
                    .from('orders')
                    .select('id, scheduled_at, reminder_error_log, clients(name, phone), professionals(name)')
                    .eq('barbershop_id', barbershopId)
                    .eq('reminder_failed', true)
    ...
    const generateWhatsAppLink = (log) => {
        ...
        const baseMsg = messages.reminder || "Olá {{cliente}}, seu horário com {{barbeiro}} está próximo! Falta cerca de 1 hora.";
    ...
    const handleResolveError = async (orderId) => {
        try {
            const { error } = await supabase
                .from('orders')
                .update({ reminder_failed: false, reminder_error_log: null })
                .eq('id', orderId);
            if (error) throw error;
                                            ...
                                            {failedLogs.map(log => {
                                                ...
                                                return (
                                                    <tr key={log.id} className="hover:bg-slate-800/30 transition-colors group">
                                                        ...
                                                        <td className="px-4 py-4">
                                                            <div className="text-rose-400 text-xs line-clamp-3 leading-relaxed" title={log.reminder_error_log}>
                                                                {log.reminder_error_log || 'Falha desconhecida no webhook de envio.'}
                                                            </div>
                                                        </td>
                                                        <td className="px-4 py-4 text-right">
                                                            <div className="flex justify-end gap-2 opacity-100 sm:opacity-50 sm:group-hover:opacity-100 transition-opacity">
                                                                {phone ? (
                                                                    <a href={generateWhatsAppLink(log)} target="_blank" rel="noopener noreferrer" className="p-2 bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 rounded-lg transition-colors" title="Enviar mensagem pré-preenchida">
